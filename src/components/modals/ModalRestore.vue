  ...
  <v-f-modal-layout>
    <div class="sm:flex sm:items-start">
      ...
      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
        <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-400" id="modal-title">{{ t('Restore files') }}</h3>
        <div class="mt-2">
          <p class="text-sm text-gray-500">{{ t('Are you sure you want to restore these files?') }}</p>
...
const remove = () => {
  ...
  if (items.value.length) {
    emitter.emit('vf-fetch', {
      ...
      onSuccess: () => {
        emitter.emit('vf-toast-push', {label: t('Files restored.')});
      },
      onError: (e) => {
